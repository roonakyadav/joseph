...
function AdminDashboard() {
  ...
  return <>
    <PageHeading eyebrow="APEX / Operations" title="Archive control room" note="A live record of controlled inventory, seller review, evidence publication, and public contact configuration." />
...
function AdminAccounts() {
  ...
  return <>
    <PageHeading eyebrow="APEX / Inventory" title="Account records" note="Create controlled drafts, attach verified media, publish deliberately, and archive rather than delete operational history." action={<ActionButton onClick={() => setEditing(null)}><Plus className="h-3 w-3" />New account</ActionButton>} />
...
function AdminSubmissions() {
  ...
  return <>
    <PageHeading eyebrow="APEX / Seller review" title="Submission queue" note="Seller submissions are private operational records. Progress status without promising approval; convert only after your own verification." />
...
function AdminProofs() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!file) { setError("Choose a redacted image before creating a proof record."); return; } setError(""); try { const image = await imageFileToUpload(file, "Redacted APEX sale proof"); await create.mutateAsync({ accountId: accountId || null, ovr: ovr ? Number(ovr) : null, image, kind, caption: caption || null, isDevelopment }); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to create proof record."); } };

  return <>
    <PageHeading eyebrow="APEX / Evidence" title="Proof control" note="Create proof records privately, redact identifying information before upload, and publish only authentic non-development evidence." action={<ActionButton onClick={() => setFormOpen(value => !value)}><Plus className="h-3 w-3" />New proof</ActionButton>} />
...
function AdminSettings() {
  ...
  const [form, setForm] = useState({ storeName: "APEX", whatsappNumber: "", whatsappCommunityUrl: "", defaultCurrency: "USD" });
