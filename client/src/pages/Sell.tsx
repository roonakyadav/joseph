...
function validateDraft(draft: SellerSubmissionDraft, imageCount: number): ValidationErrors {
  ...
  if (!draft.sellerName.trim()) errors.sellerName = "Add the name you want Elite Traders to use.";
...
export default function Sell() {
  usePageMeta({ title: "Sell an FC Mobile account — Elite Traders", description: "Submit an FC Mobile account record and supporting images for private Elite Traders review. Submissions do not self-publish a listing.", path: "/sell" });
  ...
  return <div className="sell-page"><div className="sell-grid" aria-hidden="true" /><ApexCatalogHeader active="sell" /><main className="sell-main">
    <section className="sell-intro" aria-labelledby="sell-title"><div className="sell-rail"><p className="eyebrow"><i /> Elite Traders / Seller intake</p><span>Review protocol / 03</span></div><div className="sell-intro-copy"><h1 id="sell-title">Prepare an<br /><em>account record.</em></h1><p>Compile squad detail and account evidence for review. No record is published until Elite Traders makes an explicit decision.</p><button type="button" className="sell-primary focus-ring" onClick={startSubmission}>Open account record <ArrowDown size={16} /></button></div><aside className="sell-intake-stamp"><span className="sell-a-seal" aria-hidden="true"><img src="/manus-storage/apex-mark_890b511d.png" alt="" /></span><p>Submission review</p><strong>Not a self-publish route</strong><small>Prepared account records are reviewed before any public decision.</small></aside></section>
    <section className="sell-process" aria-labelledby="process-title"><div className="process-heading"><span>001</span><p id="process-title">What happens next</p></div><ol>{[["01", "Share account details", "Create the core record Elite Traders needs to review."], ["02", "Add supporting screenshots", "Include clear account and squad images."], ["03", "Elite Traders reviews the submission", "The information is considered before any listing decision."], ["04", "Continue through direct contact", "Your chosen contact channel is used if follow-up is needed."]].map(([number, title, description]) => <li key={number}><span>{number}</span><div><strong>{title}</strong><p>{description}</p></div></li>)}</ol></section>
    <form ref={formRef} className="seller-form" onSubmit={submit} noValidate aria-labelledby="form-title"><div className="form-header"><div><p className="eyebrow">002 / Assemble account file</p><h2 id="form-title">Submission record</h2></div><p>Fields marked <b>Required</b> are needed for Elite Traders to review your account record.</p></div>
      ...
      <div className="form-submit"><p><i /> Submitting sends an account for Elite Traders review. It does not create or approve a public listing.</p><button className="sell-primary focus-ring" type="submit" disabled={submitState === "submitting"}>{submitState === "submitting" ? <><Loader2 className="spin" size={16} /> Preparing handoff</> : <>Submit for review <ChevronRight size={16} /></>}</button></div>
    </form>
  </main></div>;
}
