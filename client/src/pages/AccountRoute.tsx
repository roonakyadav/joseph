  ...
  return <>
    <section className="record-gallery" aria-label={`${title} media gallery`} onPointerDown={onPointerDown} onPointerUp={onPointerUp}>
      <button ref={openButton} className="record-gallery-main focus-ring" type="button" onClick={() => setViewerOpen(true)} aria-label={`Expand ${activeMedia.label.toLowerCase()} for ${title}`}>
        ...
        {imageState === "missing" ? <span className="gallery-missing"><i /> Record image unavailable</span> : <img src={activeMedia.src} alt={activeMedia.alt} onLoad={() => setImageState("ready")} onError={() => setImageState("missing")} />}
      ...
      {canNavigate && <div className="gallery-thumbnails" aria-label="Choose gallery image">{media.map((item, index) => <button type="button" key={item.src} className={`focus-ring ${index === activeIndex ? "is-active" : ""}`} onClick={() => setActiveIndex(index)} aria-label={`Show image ${index + 1}`}><img src={item.src} alt="" /></button>)}</div>}
    </section>
