    ...
    <article className={`account-card ${isSold ? "is-sold" : ""}`}>
      <Link href={`/accounts/${account.slug}`} className="account-card-link focus-ring" aria-label={`View ${account.title}, ${account.id}`}>
        <div className="account-card-image">
          ...
          <div className="account-card-topline"><span className="dev-mark">Concept preview</span><span className={`status-mark ${account.status}`}>{account.status}</span></div>
        ...
        <div className="account-card-body">
          ...
          <p className="record-classification">Development account record / Not verified live inventory</p>
