  ...
  return (
    <>
      <header className="catalog-header">
        ...
        <div className="catalog-header-meta"><span>Archive / Live</span><button className="catalog-index-trigger focus-ring" type="button" onClick={() => setIndexOpen(true)} aria-expanded={indexOpen} aria-controls="catalog-index"><i /> Index <b>{active === "proofs" ? "04" : active === "sell" ? "03" : active === "accounts" ? "02" : "01"}</b></button></div>
      </header>
      <aside id="catalog-index" className={`catalog-index ${indexOpen ? "is-open" : ""}`} aria-hidden={!indexOpen}>
        ...
        <nav className="catalog-index-nav" aria-label="APEX platform routes">
          ...
          <Link href="/accounts" className={active === "accounts" ? "is-active" : ""} onClick={() => setIndexOpen(false)}><span>02</span><strong>Accounts</strong><small>Public record index</small>{active === "accounts" && <i>Active</i>}</Link>
