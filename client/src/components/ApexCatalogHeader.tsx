...
export function ApexCatalogHeader({ active = "accounts" }: { active?: "home" | "accounts" }) {
  ...
  return (
    <>
      <header className="catalog-header">
        ...
        <div className="catalog-header-meta"><span>Build / 03</span><button className="catalog-index-trigger focus-ring" type="button" onClick={() => setIndexOpen(true)} aria-expanded={indexOpen} aria-controls="catalog-index"><i /> Index <b>{active === "accounts" ? "02" : "01"}</b></button></div>
      </header>
      <aside id="catalog-index" className={`catalog-index ${indexOpen ? "is-open" : ""}`} aria-hidden={!indexOpen}>
        ...
        <nav className="catalog-index-nav" aria-label="APEX platform routes">
          ...
          <div><span>03</span><strong>Sell</strong><small>Submission protocol</small></div>
          <div><span>04</span><strong>Proofs</strong><small>Verification log</small></div>
